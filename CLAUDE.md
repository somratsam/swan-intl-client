...
15. **On this machine, a network-call failure has had more than one root cause — verify empirically before picking one, don't pattern-match to the last incident.** (a) A 5-second HTTPS POST timeout that looked like a blocked connection was actually memory pressure — a clean reboot fixed it, no code change. (b) A later `forgot-password` email send to Resend kept failing even after freeing RAM from ~350MB to ~1.15GB free — that one turned out to be unrelated to memory: Node's built-in `fetch` (undici) has a hardcoded 10-second connect timeout, and this machine's real handshake latency to `api.resend.com` was ~22s (confirmed by comparing a raw Node `fetch` test against PowerShell's `Invoke-WebRequest`, which succeeded at ~22.7s using a more patient client). Before concluding "it's RAM" or "it's a firewall block": check free RAM, **and** cross-check with an independent tool/path (PowerShell `Invoke-WebRequest`/`Test-NetConnection` vs `curl`, or a raw Node `fetch` test) so the two hypotheses can actually be told apart, rather than stopping at whichever one matches a prior incident.
16. **Node's built-in `fetch` (undici) on this backend has a hardcoded 10-second connect timeout with no per-call override**, and `node:undici` is not importable as a builtin on this machine's Node version (v24) — raising it would require adding `undici` as an explicit dependency and calling `setGlobalDispatcher`. Not done. `auth.service.ts`'s `forgotPassword` works around the user-facing symptom by not awaiting the email send (so a slow/failed connect can no longer block the HTTP response), but the underlying timeout can still cause a real delivery to silently fail on a slow connection — worth knowing if "the reset email never arrived" comes up again.
