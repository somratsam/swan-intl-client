...
const stats = [
  { label: 'Brands', key: 'brands', href: '/admin/brands', color: '#C9A84C', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z' },
...
export default function AdminDashboard() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-normal" style={{ fontFamily: 'Playfair Display, serif', color: '#fff' }}>Dashboard</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          ...
          >
            <Link
              href={stat.href}
              className="block p-6 border transition-all duration-200 hover:border-[#2a2a2a] group"
            ...
            >
              <div className="flex items-start justify-between mb-4">
                ...
                <svg width="14" height="14" fill="none" stroke="#333" strokeWidth="2" viewBox="0 0 24 24" className="group-hover:stroke-[#888] transition-colors">
              ...
              <p className="text-xs tracking-[2px] uppercase" style={{ color: '#555' }}>{stat.label}</p>
            </Link>
          </motion.div>
        ))}
      </div>
      ...
      {/* Quick actions */}
      <div className="mt-10 p-6 border" style={{ background: '#0d0d0d', borderColor: '#1a1a1a' }}>
        <p className="text-xs tracking-[3px] uppercase mb-5" style={{ color: '#C9A84C' }}>Quick Actions</p>
        <div className="flex flex-wrap gap-3">
          {stats.map((s) => (
            <Link key={s.key} href={s.href} className="text-[10px] tracking-[2px] uppercase px-4 py-2 border hover:border-[#C9A84C] hover:text-white transition-all" style={{ borderColor: '#222', color: '#666' }}>
              + {s.label}
            </Link>
          ))}
        </div>
