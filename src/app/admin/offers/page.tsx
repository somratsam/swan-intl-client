...
export default function AdminOffersPage() {
  ...
  const InputField = ({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) => (
    <div>
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-4 py-2.5 text-sm bg-transparent border outline-none focus:border-[#C9A84C] transition-colors" style={{ borderColor: '#222', color: '#fff' }} />
    </div>
  );
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-normal" style={{ fontFamily: 'Playfair Display, serif', color: '#fff' }}>Offers</h1>
      ...
      {isLoading ? (
        ...
        <div className="border" style={{ borderColor: '#1a1a1a' }}>
          <table className="w-full text-sm">
            ...
            <tbody>
              {offers.map((o) => (
                <tr key={o._id} style={{ borderBottom: '1px solid #1a1a1a' }} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1 flex-wrap">
                      {o.tags?.slice(0, 2).map((t) => <span key={t} className="text-[9px] px-2 py-0.5" style={{ background: '#1a1a1a', color: '#666' }}>{t}</span>)}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-3">
                      <button onClick={() => openEdit(o)} className="text-xs hover:text-white transition-colors" style={{ color: '#C9A84C' }}>Edit</button>
                      <button onClick={() => setDeleteId(o._id)} className="text-xs hover:text-red-400 transition-colors" style={{ color: '#666' }}>Delete</button>
                    </div>
                  </td>
                </tr>
              ))}
              {offers.length === 0 && <tr><td colSpan={5} className="px-4 py-12 text-center text-sm" style={{ color: '#555' }}>No offers yet.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center px-4" style={{ background: 'rgba(0,0,0,0.85)' }} onClick={() => setModalOpen(false)}>
            <motion.div initial={{ scale: 0.93 }} animate={{ scale: 1 }} exit={{ scale: 0.93 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-2xl p-8 border overflow-y-auto max-h-[90vh]" style={{ background: '#111', borderColor: '#222' }}>
              ...
              <div className="space-y-4">
                ...
                <p className="text-[10px] tracking-[3px] uppercase pt-2" style={{ color: '#C9A84C' }}>Contact</p>
                ...
                <p className="text-[10px] tracking-[3px] uppercase pt-2" style={{ color: '#C9A84C' }}>Social Media</p>
                <div className="grid grid-cols-3 gap-4">
                ...
                <p className="text-[10px] tracking-[3px] uppercase pt-2" style={{ color: '#C9A84C' }}>Exclusive Details</p>
                <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-[10px] tracking-[3px] uppercase mb-2" style={{ color: '#888' }}>Tags (comma separated)</label>
                  <input value={(editing.tags ?? []).join(', ')} onChange={(e) => setEditing({ ...editing, tags: e.target.value.split(',').map((t) => t.trim()).filter(Boolean) })} className="w-full px-4 py-2.5 text-sm bg-transparent border outline-none focus:border-[#C9A84C] transition-colors" style={{ borderColor: '#222', color: '#fff' }} />
                </div>
              </div>
