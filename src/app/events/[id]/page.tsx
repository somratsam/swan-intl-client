    ...
    <div style={{ background: '#0a0a0a', minHeight: '100vh' }}>
      ...
      <div className="relative h-[60vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-end pb-16 px-6 text-center z-10">
          <span className="text-[9px] tracking-[3px] uppercase px-4 py-2 mb-5 font-semibold" style={{ background: 'rgba(201,168,76,0.92)', color: '#000' }}>
            {event.category}
          </span>
      ...
      <div className="max-w-4xl mx-auto px-6 py-20">
          ...
          {[
            ...
            <div key={label}>
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <p className="text-[9px] tracking-[3px] uppercase" style={{ color: '#C9A84C' }}>{label}</p>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-14">
          <p className="text-base leading-loose" style={{ color: '#888', lineHeight: '1.85' }}>{event.description}</p>
        </motion.div>

        {/* Organizer */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="p-8 border mb-10" style={{ background: '#0d0d0d', borderColor: '#1a1a1a' }}>
          <p className="text-[9px] tracking-[3px] uppercase mb-6" style={{ color: '#C9A84C' }}>Organizer</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="flex items-start gap-2.5">
              <Mail size={13} className="mt-0.5 shrink-0" style={{ color: '#C9A84C' }} />
              <div>
            ...
            <div className="flex items-start gap-2.5">
              <Phone size={13} className="mt-0.5 shrink-0" style={{ color: '#C9A84C' }} />
              <div>
        ...
        <div className="flex flex-wrap gap-3">
          {event.tags.map((tag) => (
            <span key={tag} className="text-[9px] tracking-[2px] uppercase px-4 py-2" style={{ border: '1px solid #1e1e1e', color: '#555' }}>
              {tag}
            </span>
          ))}
        </div>
